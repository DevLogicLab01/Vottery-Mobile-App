-- Fix migration: ensure all tables from 20260301030000 exist with correct columns
-- This migration is idempotent and safe to run even if the base migration failed
...
ALTER TABLE public.cache_roi_metrics
  ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();
...
ALTER TABLE public.performance_tuning_recommendations
  ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();
...
ALTER TABLE public.incident_response_log
  ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();
